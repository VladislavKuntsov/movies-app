...
export default class MoviesList extends Component {
    ...
    render() {
        const { movies, loadingIndicator, error, sendMoveRatingItem, switchSearchRate, moviesListRating, rating } = this.props;
        ...
        const moviesAvailability = !hasData ? < MoviesView movies={movies} sendMoveRatingItem={sendMoveRatingItem} rating={rating} /> : null;
 ...
 const MoviesView = ({ movies, sendMoveRatingItem, rating }) => {
     const cards = movies.map(({ id, ...items }) => {
            const card = <MovieItem
                key={id}
                {...items}
                sendMoveRatingItem={sendMoveRatingItem}
                rating={rating}
                id={id}
            />
        return card
